        ...
        try {
            ...
            try {
                ...
                if (notificationCreated) {
                    System.out.println("✅ Notification créée avec succès pour le fermier de la ferme #" + newAnalyse.getIdFerme());
                } else {
                    System.err.println("⚠️ Impossible de créer la notification pour la ferme #" + newAnalyse.getIdFerme() + " (fermier non trouvé ou erreur DB)");
                }
            } catch (Exception ex) {
    ...
    private void handleAddAnalyse() {
        ...
        result.ifPresent(newAnalyse -> {
            try {
                ...
                // This ensures the fermier sees the notification even if they're not currently logged in
                try {
                    NotificationService notificationService = new NotificationService();
                    boolean notificationCreated = notificationService.createForFermier(
                        newAnalyse.getIdFerme(),
